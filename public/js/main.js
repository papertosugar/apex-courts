...
// ─── AVAILABILITY GRID ────────────────────────────────────────
const TIMES  = ['6AM','7AM','8AM','9AM','10AM','11AM','12PM','1PM','2PM','3PM','4PM','5PM','6PM','7PM','8PM','9PM','10PM','11PM'];
...
async function renderAvailability(sport) {
  ...
  // Time header — spacer must match .court-label width exactly
  const labelSize = isDZ ? '7px' : '9px';
  let html = `<div style="display:flex;gap:4px;margin-bottom:8px">
    <div style="width:88px;min-width:88px;flex-shrink:0"></div>
    <div style="display:flex;gap:4px;flex:1;min-width:0">`;
  timeArr.forEach(t => {
    const h = parseHourStr(t);
    const isCurrent = Math.floor(h) === Math.floor(nowH);
    html += `<div style="flex:1;min-width:0;font-size:${labelSize};text-align:center;letter-spacing:0.02em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
